...
DEFAULT_TYPES = [
    ...
    },
    {
        "name": "project",
        "display_name": "プロジェクト",
        "description": "担当するアプリ開発プロジェクト",
        "fields_schema": [
            {"name": "repo_url", "type": "string", "description": "リポジトリURL"},
            {"name": "tech_stack", "type": "string", "description": "技術スタック（言語、フレームワーク等）"},
            {"name": "project_status", "type": "string", "description": "ステータス (planning/active/maintenance/archived)"},
            {"name": "environment_url", "type": "string", "description": "デプロイ先URL（本番・ステージング等）"},
            {"name": "start_date", "type": "date", "description": "開始日"},
            {"name": "members", "type": "ref", "ref_type": "person", "multiple": True, "description": "プロジェクトメンバーのアイテムID"},
        ],
    },
    {
        "name": "meeting",
        ...
        "fields_schema": [
            {"name": "meeting_date", "type": "date", "description": "開催日"},
            {"name": "start_time", "type": "time", "description": "開始時刻 (HH:MM)"},
            {"name": "end_time", "type": "time", "description": "終了時刻 (HH:MM)"},
            {"name": "meeting_type", "type": "string", "description": "種別 (standup/sprint/1on1/review/retrospective/other)"},
            {"name": "attendees", "type": "ref", "ref_type": "person", "multiple": True, "description": "参加者のアイテムID"},
            {"name": "action_items", "type": "string", "description": "アクションアイテム"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
        ],
    },
    {
        "name": "bug",
        "display_name": "バグ",
        "description": "発見されたバグ・不具合の記録",
        "fields_schema": [
            {"name": "severity", "type": "string", "description": "重要度 (critical/high/medium/low)"},
            {"name": "bug_status", "type": "string", "description": "ステータス (open/investigating/fixing/resolved/wontfix)"},
            {"name": "steps_to_reproduce", "type": "string", "description": "再現手順"},
            {"name": "environment", "type": "string", "description": "発生環境（OS、ブラウザ、バージョン等）"},
            {"name": "found_date", "type": "date", "description": "発見日"},
            {"name": "resolved_date", "type": "date", "description": "解決日"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
            {"name": "assignee", "type": "ref", "ref_type": "person", "description": "担当者のアイテムID"},
        ],
    },
    {
        "name": "idea",
        "display_name": "アイデア",
        "description": "機能アイデアや改善提案",
        "fields_schema": [
            {"name": "category", "type": "string", "description": "カテゴリ (feature/improvement/refactoring/performance/ux)"},
            {"name": "impact", "type": "string", "description": "インパクト (high/medium/low)"},
            {"name": "effort", "type": "string", "description": "工数見積もり (small/medium/large)"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
        ],
    },
    {
        ...
        "description": "PR/MRレビューの記録と追跡",
        "fields_schema": [
            {"name": "pr_url", "type": "string", "description": "PR/MRのURL"},
            {"name": "review_status", "type": "string", "description": "ステータス (pending/approved/changes_requested/merged)"},
            {"name": "review_date", "type": "date", "description": "レビュー日"},
            {"name": "reviewer", "type": "ref", "ref_type": "person", "description": "レビュアーのアイテムID"},
            {"name": "author", "type": "ref", "ref_type": "person", "description": "PR作成者のアイテムID"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
        ],
    },
    {
        "name": "sprint",
        "display_name": "スプリント",
        "description": "スプリント期間の計画と実績の管理",
        "fields_schema": [
            {"name": "sprint_number", "type": "string", "description": "スプリント番号"},
            {"name": "start_date", "type": "date", "description": "開始日"},
            {"name": "end_date", "type": "date", "description": "終了日"},
            {"name": "sprint_goal", "type": "string", "description": "スプリントゴール"},
            {"name": "velocity", "type": "string", "description": "ベロシティ（完了ポイント数）"},
            {"name": "sprint_status", "type": "string", "description": "ステータス (planning/active/review/completed)"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
        ],
    },
    {
        "name": "deployment",
        "display_name": "デプロイ",
        "description": "デプロイ実施の記録と追跡",
        "fields_schema": [
            {"name": "deploy_date", "type": "date", "description": "デプロイ日"},
            {"name": "deploy_time", "type": "time", "description": "デプロイ時刻 (HH:MM)"},
            {"name": "target_environment", "type": "string", "description": "デプロイ先環境 (production/staging/development)"},
            {"name": "version", "type": "string", "description": "デプロイバージョン"},
            {"name": "deploy_status", "type": "string", "description": "ステータス (planned/in_progress/success/failed/rolled_back)"},
            {"name": "rollback_reason", "type": "string", "description": "ロールバック理由"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
            {"name": "deployer", "type": "ref", "ref_type": "person", "description": "デプロイ担当者のアイテムID"},
        ],
    },
    {
        "name": "spec",
        "display_name": "仕様",
        "description": "機能仕様・要件定義・設計ドキュメント",
        "fields_schema": [
            {"name": "spec_type", "type": "string", "description": "種別 (feature/api/design/requirement)"},
            {"name": "spec_status", "type": "string", "description": "ステータス (draft/review/approved/implemented/obsolete)"},
            {"name": "priority", "type": "string", "description": "優先度 (high/medium/low)"},
            {"name": "acceptance_criteria", "type": "string", "description": "受け入れ基準"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
            {"name": "author", "type": "ref", "ref_type": "person", "description": "作成者のアイテムID"},
        ],
    },
    {
        "name": "tech_debt",
        "display_name": "技術的負債",
        "description": "リファクタリングや改善すべき技術的課題の管理",
        "fields_schema": [
            {"name": "debt_type", "type": "string", "description": "種別 (code_quality/architecture/dependency/performance/security/test_coverage)"},
            {"name": "severity", "type": "string", "description": "深刻度 (critical/high/medium/low)"},
            {"name": "effort", "type": "string", "description": "対応工数見積もり (small/medium/large/xlarge)"},
            {"name": "debt_status", "type": "string", "description": "ステータス (identified/planned/in_progress/resolved/accepted)"},
            {"name": "affected_area", "type": "string", "description": "影響箇所（モジュール名やファイルパス）"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
            {"name": "assignee", "type": "ref", "ref_type": "person", "description": "担当者のアイテムID"},
        ],
    },
    {
        "name": "release",
        "display_name": "リリース",
        "description": "アプリのリリース記録",
        "fields_schema": [
            {"name": "version", "type": "string", "description": "バージョン番号"},
            {"name": "release_date", "type": "date", "description": "リリース日"},
            {"name": "release_type", "type": "string", "description": "種別 (major/minor/patch/hotfix)"},
            {"name": "changelog", "type": "string", "description": "変更内容"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
            {"name": "deployer", "type": "ref", "ref_type": "person", "description": "デプロイ担当者のアイテムID"},
        ],
    },
    {
        "name": "incident",
        "display_name": "インシデント",
        "description": "本番障害・サービス影響の記録",
        "fields_schema": [
            {"name": "incident_level", "type": "string", "description": "レベル (sev1/sev2/sev3/sev4)"},
            {"name": "incident_status", "type": "string", "description": "ステータス (investigating/identified/mitigated/resolved)"},
            {"name": "occurred_at", "type": "date", "description": "発生日時"},
            {"name": "resolved_at", "type": "date", "description": "解決日時"},
            {"name": "impact_scope", "type": "string", "description": "影響範囲"},
            {"name": "root_cause", "type": "string", "description": "根本原因"},
            {"name": "action_taken", "type": "string", "description": "対応内容"},
            {"name": "prevention", "type": "string", "description": "再発防止策"},
            {"name": "related_project", "type": "ref", "ref_type": "project", "description": "関連プロジェクトのアイテムID"},
            {"name": "responders", "type": "ref", "ref_type": "person", "multiple": True, "description": "対応者のアイテムID"},
        ],
    },
]
